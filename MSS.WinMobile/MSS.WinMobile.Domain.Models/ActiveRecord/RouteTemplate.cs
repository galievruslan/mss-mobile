...
namespace MSS.WinMobile.Domain.Models
{
    public partial class RouteTemplate : ActiveRecordBase
    {
        ...
        protected override string InsertCommand {
            get
            {
                return string.Format("INSERT INTO [{0}] ([{1}], [{2}], [{3}]) VALUES ({4}, {5}, {6})",
                                     Table.TABLE_NAME, Table.Fields.ID, Table.Fields.DAY_OF_WEEK, Table.Fields.MANAGER_ID, Id, DayOfWeek, ManagerId);
            }
        }
        ...
        protected override string UpdateCommand {
            get
            {
                return string.Format("UPDATE [{0}] SET [{1}] = {2}, " +
                                     "[{3}] = {4} " +
                                     "WHERE [{5}] = {6}",
                                     Table.TABLE_NAME, Table.Fields.DAY_OF_WEEK, DayOfWeek,
        ...
        public static RouteTemplate GetByDate(DateTime date)
        {
            return 
                QueryObjectFactory.CreateQueryObject<RouteTemplate>()
                                  .Where(Table.Fields.DAY_OF_WEEK, new Equals((int)date.DayOfWeek))
                                  .FirstOrDefault();
        }
    }
}
