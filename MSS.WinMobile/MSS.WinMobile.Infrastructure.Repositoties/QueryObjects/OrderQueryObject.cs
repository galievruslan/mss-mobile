...
namespace MSS.WinMobile.Infrastructure.SqliteRepositoties.QueryObjects
{
    ...
    {
        public OrderQueryObject(ITranslator<Order> translator) : base(translator)
        {
        }

        public OrderQueryObject(IQueryObject<Order, string> queryObject)
            : base(queryObject)
        {
        }

        private const string SelectQuery =
            "SELECT orders.Id, orders.OrderDate, orders.ShippingDate, orders.Manager_Id, " +
