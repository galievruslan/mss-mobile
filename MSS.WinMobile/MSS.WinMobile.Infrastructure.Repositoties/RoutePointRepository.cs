...
namespace MSS.WinMobile.Infrastructure.Sqlite.Repositoties
{
    public class RoutePointStorageRepository : SqLiteStorageRepository<RoutePoint> {
        ...
        private const string SaveQueryTemplate = "INSERT OR REPLACE INTO RoutePoints (Id, Route_Id, ShippingAddress_Id, ShippingAddress_Name, ShippingAddress_Address, Status_Id, Status_Name, Synchronized) VALUES ({0}, {1}, {2}, '{3}', '{4}', {5}, '{6}', {7})";
        protected override string GetSaveQueryFor(RoutePoint model) {
            return string.Format(SaveQueryTemplate,
                                 model.Id != 0
                                     ...
                                     : "NULL", model.RouteId, model.ShippingAddressId,
                                 model.ShippingAddressName.Replace("'", "''"),
                                 model.ShippingAddressAddress.Replace("'", "''"), model.StatusId,
                                 model.StatusName.Replace("'", "''"), model.Synchronized ? 1 : 0);
        }
